...
class Card:                 #Создание класса "Карты"
    ...
    values=["None","None","2","3","4","5","6","7","8","9","10","вальта","даму","короля","туза"] #Создание переменной, включающей в себя номиналы карт.
...
class Game:
    ...
    def play_game(self):
        ...
        print("Поехали!")
        while len(cards)>=2:
            if (len(self.deck.cards)//10)%10==1:
                end= ""
            else:
                if len(self.deck.cards)%10==0 or 5<len(self.deck.cards)%10<=9:
                    end= ""
                elif len(self.deck.cards)%10==1:
                    end= "а"
                elif 1<len(self.deck.cards)%10<=4:
                    end= "ы"
            print("Текущий счет {}-{} : {}-{}. В колоде осталось {} карт{}".\
                  format(self.p1.name,self.p1.wins,self.p2.name,self.p2.wins,\
                         len(self.deck.cards),end))
